...
- One call `/database/analytics` (filters: agency, client, monthFrom/To, **brand**). Returns `totalValue`, `totalWithVat`, and arrays `byMonth`, `byMedium`, `byMediaGroup`, `byChannel` (now carries `channelMasterId`), `byClient` (now carries `clientId`), `byBrand`, `byAgency`, plus derived `byMonthMedium`, `brandTrend`+`brandTrendKeys`, `clientTenure`, `clientFlighting`+`flightingMonths`, and **`brandOptions`** (distinct brand names for the selected client's scope).
- **Brand drill-down:** selecting a single **Client** reveals a **Brand** filter in the hero (populated from `brandOptions`); picking a brand adds `?brand=` to the analytics fetch so **every chart/table respects it**. `getAnalytics` computes `brandOptions` from the client scope *before* applying the brand filter, so the picker stays populated once a brand is chosen; the brand clears whenever the client changes. Brand is a `ScheduleLog.brandName` dimension, so it scopes the charts only — the Deals & Properties and Client Targets sections are unaffected.
- **Agency overview collapses under a client filter:** when a Client is selected, the agency-level views (agency **Annual Achievement** + **Spend by Agency**) are hidden behind an **"Agency overview"** expand toggle (`agencyOverviewOpen`), since they aren't client-specific; with no client selected they render normally.
...
- **Agency target proration over active months:** the Agency Performance cards (Executive Dashboard) and the Spend Analytics agency **Annual Achievement** prorate an agency's annual target over its OPERATING months, not from January — a mid-year starter (e.g. RedWorks from May) uses target ÷ (active months) × (active months elapsed), detected per agency from its first month with `ScheduleLog` data. Agency Performance cards also: count Active Clients with ≥1 log **this year**, show a target progress bar, and are centered.
...
- **Sidebar brand lockup:** the sidebar shows the **animated orbit mark + "Ogilvy Orbit" on ONE line** (`Layout.jsx` `.brand`, `.brand-name` in `index.css`), set in **Ogilvy Sans** via `--brand-font`. It deliberately does **NOT** use `web/public/brand-logo.*` — that file brands the generated documents only, so changing the document logo never touches the app chrome. (Earlier iterations rendered the image here; that was removed on request.)
- **Ogilvy Sans** is proprietary, so it is **not** loaded from Google Fonts/any CDN. `index.css` declares `@font-face` for `/fonts/OgilvySans-{Regular,Medium,Bold}.woff2` — drop the licensed files at those paths in `web/public/fonts/` and they're picked up with no code change. Until then the browser skips the missing source and falls back to Hanken Grotesk via `--brand-font: "Ogilvy Sans", var(--font)`; a viewer with the font installed locally gets it either way. Swap `--font` to `--brand-font`'s value to take it app-wide.
- **Document logo (every export):** dropping **one** file — `web/public/brand-logo.png` (preferred — transparent) or `brand-logo.jpg` — brands **every generated document**. There is deliberately no second place to configure it.
  - **Single source of truth:** `web/src/lib/brandLogo.js` owns the file list — `BRAND_LOGO_FILES` (`['/brand-logo.png','/brand-logo.jpg']`), with `BRAND_LOGO_CANDIDATES` appending `/orbit-logo.png` so documents stay branded before a custom logo is added. Consumed by the exports only; the sidebar does not import it. `loadBrandLogo()` caches per page load, verifies the response is really an image (an SPA fallback can answer a missing file with `index.html`), and returns `{dataUrl, width, height, ratio, format}`; `fitLogo()` scales into a box **without distorting** any aspect ratio; `drawPdfLogo()` places it in a jsPDF header. Server side, `api/src/utils/brandLogo.js` reads the **same files** out of `web/dist` (what Express serves; falls back to `web/public` in dev) and sniffs PNG/JPEG headers for intrinsic size — no image library, and the JPEG walker handles progressive (SOF2) files. Both return `null` when nothing loads, and every call site treats the logo as optional, so **a missing or corrupt logo can never break an export**.
  - **Header treatment:** the logo **replaces the whole "Ogilvy Orbit" lockup** (both the coral "O" mark and the wordmark); the report subtitle stays. With no logo file the original drawn lockup renders exactly as before.
  - **White chip on dark bands:** a **JPG cannot carry transparency**, so on the navy header bands (server PDFs, the Executive Summary PDF, the PPT cover/closing slides) the logo is drawn on a white rounded panel — an opaque logo then reads as a deliberate badge instead of a stray white rectangle. Light backgrounds get no chip.
  - **Where it appears:** **9 PDFs** — 3 server-side PDFKit (`generatePdf`, `generatePropertyHistoryPdf`, `generateGroupedTablePdf` via the shared `drawPdfBrand` helper) and 6 client-side jsPDF (MBR requisition, Executive Summary, Spend Analytics, Forecasting Insights, Deep Dashboard ×2); the **PPTX** (cover, every chart slide, closing slide); **server-side Excel** (`generateExcel` via `addSheetLogo`); and **10 client-side Excel** exports.
  - **Client-side Excel moved from SheetJS to ExcelJS** — SheetJS cannot embed images at all. `web/src/lib/brandedXlsx.js` is the adapter: `writeBrandedWorkbook([{name, aoa|json, cols}], fileName)` takes the same arrays the call sites already build, so each conversion was one line rather than a rewrite. It sanitises sheet names (Excel forbids `: \ / ? * [ ]`, 31-char cap), accepts SheetJS `{wch}` widths or plain numbers, and **dynamically imports** `exceljs` so the ~930 kB library is a separate chunk — the initial bundle grew only ~2.6 kB. `xlsx` is still used everywhere for **reading** uploads; only writes changed.
  - **Three exports deliberately keep NO logo** (do not "fix" these): the **bulk-import template** and the **failed/duplicate-row download** (`DatabasePage`) are machine-readable round-trip files that get re-uploaded — the parser expects headers on row 1, and a logo shifts them down and breaks the import; and the Forecasting **CSV** export, since the format cannot hold an image.
  - In Excel the logo is *anchored*, not in-cell, so `addSheetLogo` reserves the top rows by calling `getRow(n).height`. **`getRow()` materialises the row**, which both sets the height and reserves the space — a later `addRow()` lands underneath it, so do **not** also push empty rows or the reserve is counted twice (this was a real bug: data landed on row 9 instead of row 5).
  - **Sizing:** `fitLogo()` scales into each surface's box without distorting the aspect ratio, so no call site needs a hardcoded width/height pair.
  - **Crop whitespace out of the FILE, not with CSS.** Padding baked into the image shrinks the artwork on *every* surface (sidebar, PDFs, slides) and no CSS can recover it — `object-fit: contain` fits the whole canvas, blank margins included. The supplied logo arrived 2752×1536 with only 28% ink coverage (560px of white above the wordmark, 480px below) and rendered tiny; cropping to the ink bounding box + ~7% margin took it to 1400×319 at 85% coverage, and snapping near-white noise to pure white plus a downscale cut it 3.51 MB → 0.15 MB. Re-check this if the logo is ever replaced.
