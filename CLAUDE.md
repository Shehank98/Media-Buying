...
- **Brand logo (sidebar + every export):** dropping **one** file — `web/public/brand-logo.png` (preferred — transparent, sits better on the navy sidebar) or `brand-logo.jpg` — brands the **sidebar and every generated document**. There is deliberately no second place to configure it.
  - **Single source of truth:** `web/src/lib/brandLogo.js` owns the file list. `BRAND_LOGO_FILES` (`['/brand-logo.png','/brand-logo.jpg']`) is what `Layout.jsx`'s `BRAND_LOGOS` now imports, so the sidebar and the documents can never disagree; `BRAND_LOGO_CANDIDATES` appends `/orbit-logo.png` so **documents** stay branded before a custom logo is added (the **sidebar** instead keeps falling back to its animated orbit lockup, which is why the two lists are separate). `loadBrandLogo()` caches per page load, verifies the response is really an image (an SPA fallback can answer a missing file with `index.html`), and returns `{dataUrl, width, height, ratio, format}`; `fitLogo()` scales into a box **without distorting** any aspect ratio; `drawPdfLogo()` places it in a jsPDF header. Server side, `api/src/utils/brandLogo.js` reads the **same files** out of `web/dist` (what Express serves; falls back to `web/public` in dev) and sniffs PNG/JPEG headers for intrinsic size — no image library, and the JPEG walker handles progressive (SOF2) files. Both return `null` when nothing loads, and every call site treats the logo as optional, so **a missing or corrupt logo can never break an export**.
  ...
  - **Sidebar sizing:** `.brand-logo` (`index.css`) is `width:100%; height:auto` — the logo spans the **full sidebar width** and its height follows from its aspect ratio, with `.brand.has-logo` tightening the padding to 14px (the fallback orbit lockup keeps the original 20px). Deliberately NOT height-capped: a wide wordmark squeezed to a fixed height renders unusably small.
