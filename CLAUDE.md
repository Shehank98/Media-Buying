...
- **Per-client paste/upload** (DatabasePage): SheetJS parse, column auto-map, preview into the grid, save via `POST /api/database/bulk` (pre-resolved client/channel IDs). The file-upload preview also runs **channel reconciliation** (below): an "Auto-match channels" action on unmatched channel names, with the same map/request-new UI (client is fixed to the selected client, so only channels are reconciled); requested-new channels hold their rows the same way as the bulk import. `matchChannelByName` (grid + paste) also matches learned **aliases**, not just the exact name.
