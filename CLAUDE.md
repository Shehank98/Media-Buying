...
- Inside the Annual Achievement section, under **Monthly Spend**, three Group Contribution chart-cards in `AchievementSection` (`ExecutiveDashboardPage.jsx`), all team-grouped: each team is one `Team` (clients grouped via `TeamClient`, label includes the `headUserId` head name); clients not yet assigned to a team roll into "Unassigned". All three are read-only and unaffected by the page's year selector — each picks its own time window server-side.
  ...
  - **Group Contribution — Average vs Latest Month** (grouped bars) — `GET /dashboard/group-contribution-variance` (`getGroupContributionVariance`). Finds the single latest month with data, derives its calendar year, then averages every month **strictly before it within that same year** ("prior months") per team. Returns `{ latestMonth, latestMonthLabel, priorMonthsCount, groups: [{key, teamId, name, headName, agencyName, avgPrior, latest, diffPct}] }`. **Edge case:** if the latest month is January (no prior months in its year), `priorMonthsCount` is `0` and `groups` is empty — it deliberately does NOT reach into the previous year. `diffPct` = `(latest - avgPrior) / avgPrior * 100`, rounded to a whole number (100 if `avgPrior` is 0 and `latest` > 0). Bars: `avgPrior` (amber) vs `latest` (blue), with `diffPct` rendered as a colored (green/red) custom SVG `<text>` label next to the `latest` bar — plain CSS `style.fill` can't vary per-row, hence the custom `content` renderer on `<LabelList>`.
